/*
ricreo la funzione perche' in un punto dava un errore inspiegabile che pare sia dovuto al mal funzionamento di una query.
--> vedere 'ERROR: Condition is not correct'
*/

-- Function: public.pgr_createtopology(text, double precision, text, text, text, text, text)

-- DROP FUNCTION public.pgr_createtopology(text, double precision, text, text, text, text, text);
...
--funzione modificata da Riccardo in data 04/10/2017
...
BEGIN
  ...
  BEGIN 
    sql = 'select ' || id || ', ' || the_geom || ', ' || source || ', ' || target || ' from '||pgr_quote_ident(tabname)||' WHERE true'||rows_where ||' limit 1';
    EXECUTE sql into i;
    RAISE NOTICE '%',sql;
    sql = 'select count(*) from '||pgr_quote_ident(tabname)||' WHERE (' || gname || ' IS NOT NULL AND '||idname||' IS NOT NULL)=false '||rows_where;
    RAISE NOTICE '%',sql;
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION public.pgr_createtopology(text, double precision, text, text, text, text, text)
  OWNER TO postgres;
COMMENT ON FUNCTION public.pgr_createtopology(text, double precision, text, text, text, text, text) IS 'args: edge_table,tolerance, the_geom:=''the_geom'',source:=''source'', target:=''target'',rows_where:=''true'' - fills columns source and target in the geometry table and creates a vertices table for selected rows';
